...
RWKV_FLOAT_MODE = 'fp32'  # 'fp32', 'fp16', 'bf16'
RWKV_HEAD_QK_DIM = 0
...
class RWKV_TimeMix_Train(torch.jit.ScriptModule):
...
class RWKV_TimeMix_Infer(nn.Module):
    def __init__(self, config, layer_id):
        super().__init__()
        self.layer_id = layer_id
        self.n_embd = config.n_embd
        self.time_decay = nn.Parameter(torch.ones(config.n_embd))
        self.time_first = nn.Parameter(torch.ones(config.n_embd) * math.log(0.3))
...
######################################################################### Not needed for rwkv encoder-only versions ######################## 
